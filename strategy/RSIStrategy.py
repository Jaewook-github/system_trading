...
class RSIStrategy(QThread):
    ...
    def check_sell_signal(self, code):
        ...
        # 보유 종목의 수익율 조회
        profit_rate = self.kiwoom.balance[code]['수익률']
        print("profit_rate: ", profit_rate)

        # 금일의 RSI(2) 구하기
        rsi = df[-1:]['RSI(2)'].values[0]

        # 매도 조건 세 가지를 모두 만족하면 True
        ...
        # 보유 종목의 매입가격 조회
        purchase_price = self.kiwoom.balance[code]['매입가']
        # 현재 가격(종가)과 매입가격을 비교하여 수익률 계산
        profit_rate = (close - purchase_price) / purchase_price * 100

        # 금일의 RSI(2) 구하기
        rsi = df[-1:]['RSI(2)'].values[0]

        # 매도 조건 수정: RSI가 80 이상이고, 현재가가 매입가보다 높으며, 수익률이 1.5% 이상일 때 True 반환
        if rsi > 80 and close > purchase_price and profit_rate > 1.5:
            return True
        else:
            return False
        '''
